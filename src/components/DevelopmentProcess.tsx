  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl px-4 sm:px-6 lg:px-8 mx-auto">
        ...
        {/* Tree Root Badge / Center Start Node with Interactive Down Arrow */}
        <div className="flex flex-col items-center justify-center mb-8 relative z-20">
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className={`inline-flex items-center gap-3 px-5 sm:px-6 py-2.5 sm:py-3 rounded-2xl border text-xs sm:text-sm font-mono font-bold transition-all duration-300 shadow-xl cursor-pointer backdrop-blur-md ${
              isTreeExpanded
                ? "bg-gradient-to-r from-cyan-950/90 via-[#0d1424] to-blue-950/90 border-cyan-400 text-cyan-300 shadow-[0_0_25px_rgba(6,182,212,0.35)]"
                : "bg-slate-900/90 border-white/20 text-slate-200 hover:border-cyan-500/50 hover:bg-[#0d1424]"
            }`}
          ...
          >
            ...
            <span className="text-slate-500 font-normal hidden sm:inline">| 4 KEY PHASES</span>
            <span
              className={`p-1.5 rounded-lg bg-white/5 border border-white/10 text-cyan-400 transition-transform duration-300 ${
                isTreeExpanded ? "rotate-180 bg-cyan-500/20 text-cyan-300" : ""
              }`}
            >
              {renderIcon("FaChevronDown", { size: 12 })}
            </span>
          </motion.button>

          {/* Click hint pill when collapsed */}
          {!isTreeExpanded && (
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-[11px] font-mono text-cyan-400/80 mt-2.5 flex items-center gap-1.5 animate-pulse"
            >
              <span>Click down arrow to view 4-branch tree</span>
            </motion.span>
          )}

          {/* Glowing Stem under Root Badge when expanded */}
          {isTreeExpanded && (
            <motion.div
              initial={{ height: 0, opacity: 0 }}
              animate={{ height: 36, opacity: 1 }}
              transition={{ duration: 0.3 }}
              className="w-0.5 bg-gradient-to-b from-cyan-500 via-purple-500 to-transparent my-1"
            />
          )}
        </div>

        {/* Collapsible 4-Branch Tree Body */}
        <AnimatePresence>
          {isTreeExpanded && (
            <motion.div
            ...
            >
              ...
              {/* ==================================================================== */}
              <div className="block lg:hidden relative pl-6 sm:pl-8 pt-4">
